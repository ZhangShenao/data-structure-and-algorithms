...
/**
 * @author ZhangShenao
 * @date 2023/8/31 9:21 AM
 * @description: 以数组最后一个元素为基准值, 将数组分成三部分:左边部分元素均小于基准值;中间部分元素均等于基准值;右边部分元素均大于基准值。
 */
public class QuickSortPartition2 {
    
    public static void main(String[] args) {
        int[] arr = new int[] {7, 5, 4, 2, 4, 9, 10, 8, 4};
        System.out.println("原始数组: ");
        System.out.println(Arrays.toString(arr));
        int[] partition = partition(arr, 0, arr.length - 1);
        System.out.println("Partition返回结果: " + Arrays.toString(partition));
